...
export interface MedicalDocument {
  ...
  date?: string;
  doctorName?: string;
  category: 'Prescription' | 'Lab Report' | 'Discharge Summary' | 'Imaging Report' | 'Referral' | 'Other';
  uploadedAt?: string;
  source?: 'Scanner' | 'Upload' | 'Synthetic Sample';
  previewUrl?: string;
  facilityName?: string;
  fileType?: string;
  fileSize?: string;
  confidence?: number;
  verificationStatus: 'Unverified' | 'Patient Confirmed' | 'Staff Verified' | 'Rejected';
  extractedData?: {
    patientName?: string;
    date?: string;
    doctor?: string;
    hospital?: string;
    tests?: (string | { name: string; value: string; normalRange: string; flag: string })[];
    reportedValues?: Record<string, string>;
    diagnoses?: string[];
    diagnosesStated?: string[];
    medications?: string[];
    medicationsStated?: string[];
    allergies?: string[];
    allergiesStated?: string[];
    followUp?: string;
    followUpInfo?: string;
...
export interface ConsentRecord {
  ...
  doctorName?: string;
  status: 'DRAFT' | 'PENDING' | 'PATIENT SIGNED' | 'WITNESS SIGNED' | 'PRACTITIONER VERIFIED' | 'COMPLETED' | 'DECLINED';
  signedAt?: string;
  date?: string;
  time?: string;
...
export interface ClinicalTimelineEvent {
  ...
  eventType: string;
  summary: string;
  category: 'Visits' | 'Vitals' | 'Medications' | 'Investigations' | 'Referrals' | 'AI events';
}
...
export interface HomeopathyCase {
  id: string;
  chiefComplaint: string;
  location: string;
  sensation: string;
  ...
  thermalPreference: 'Chilly' | 'Hot' | 'Ambi-thermal';
  thirst: 'Thirsty' | 'Thirstless' | 'Small quantities often';
  appetiteDesires: string;
  appetiteAversions: string;
  sleepDreams: string;
  mentalGenerals: string;
  perspiration: string;
  remedySelected: string;
  potency: string;
...
export interface FollowUpPlan {
  ...
  department: string;
  reason: string;
  priority: 'Routine' | 'High Priority' | 'Urgent';
  instructions: string;
